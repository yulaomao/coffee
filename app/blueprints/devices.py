...
def update_device(device_no: str):
    ...
    device = merchant_scope_filter(
        Device.query.filter_by(device_no=device_no), claims
    ).first_or_404()
...
def custom_field_config():
    if request.method == "GET":
        ...
        return jsonify(
            [{"key_index": i.key_index, "enabled": i.enabled, "title": i.title} for i in items]
        )
    data = request.get_json(force=True) or {}
    if request.method == "POST":
        ...
        for i, item in enumerate(data.get("items", [])[:10], start=1):
            cfg = CustomFieldConfig(
                ...
                title=item.get("title") or f"字段{i}",
            )
            db.session.add(cfg)
        db.session.commit()
        ...
        for d in items:
            rows.append(
                [
                    ...
                    getattr(d, "address", "") or "",
                    getattr(d, "scene", "") or "",
                    getattr(d, "customer_code", "") or "",
                ]
            )
        return csv_response(
...
def device_detail(device_no: str):
    ...
    cmds = (
        RemoteCommand.query.filter_by(device_id=device.id)
            ...
            "materials": [
                {
                    ...
                    "remain": float(m.remain),
                    "capacity": float(getattr(m, "capacity", 0)),
            ...
            "command_history": [
                {
                    ...
                    "type": c.command_type,
                    "status": c.status,
                    "result_at": c.result_at.isoformat() if getattr(c, "result_at", None) else None,
                }
                for c in cmds
            ],
        }
...
def device_material_fill(device_id: int, bin_id: int):
    ...
    task_id = str(uuid.uuid4())
    submit_task(
        Task(id=task_id, type="material_fill", payload={"device_id": d.id, "bin_id": bin_id})
    )
    try:
        db.session.add(
            OperationLog(
                user_id=claims.get("id"),
                action="material_fill",
                target_type="device_material",
                target_id=dm.id,
                ip=None,
                user_agent=None,
...
def device_materials_export(device_id: int):
    ...
    try:
        ...
        db.session.rollback()
    return csv_response(
        ["bin_id", "name", "remain", "capacity", "unit", "updated_at"],
        csv_rows,
        filename=f"device_{d.device_no}_materials.csv",
    )
...
def device_orders_month(device_id: int):
    ...
    q = Order.query.filter(
        Order.device_id == d.id, Order.created_at >= start, Order.created_at < end
    )
    ...
    if view == "count":
        ...
        rows = (
            db.session.query(Order.product_name, db.func.count(Order.id))
            .filter(q_paid.whereclause)
    ...
    return jsonify(
        {"items": result_items, "summary": summary, "month": (month or start.strftime("%Y-%m"))}
    )
...
def device_orders_export(device_id: int):
    ...
    return csv_response(
        ...
        rows,
        filename=f"device_{d.device_no}_{(month or start.strftime('%Y-%m'))}_orders.csv",
    )
...
def device_cleaning_export(device_id: int):
    ...
    if not claims:
        return jsonify({"msg": "unauthorized"}), 401
    q = CleaningLog.query.filter(CleaningLog.device_id == device_id).order_by(
    ...
    rows = [
        [c.created_at.isoformat(), c.type, c.result, c.duration_ms, c.note or ""] for c in items
    ]
    try:
        db.session.add(
            OperationLog(
        ...
        db.session.rollback()
    return csv_response(
        ["created_at", "type", "result", "duration_ms", "note"],
        rows,
        filename=f"device_{device_id}_cleaning.csv",
    )
...
def device_charts_series(device_id: int):
    ...
    rows = (
        db.session.query(k.label("k"), agg.label("v"))
        .select_from(Order)
...
def device_charts_category(device_id: int):
    ...
    agg = func.sum(Order.total_amount) if metric == "amount" else func.count(Order.id)
    rows = (
        db.session.query(Order.product_name.label("k"), agg.label("v"))
        .select_from(Order)
        .filter(q.whereclause)
        .group_by("k")
        .order_by("k")
...
def cmd_set_price(device_id: int):
    ...
    rc = RemoteCommand(
        ...
        payload={"product_id": data["product_id"], "new_price": data["new_price"]},
        issued_by=claims.get("id"),
        status="pending",
    )
    db.session.add(rc)
...
def cmd_set_remaining(device_id: int):
    ...
    rc = RemoteCommand(
        command_id=cmd_id,
        device_id=d.id,
        command_type="set_remaining",
        payload={"bin_id": data["bin_id"], "new_remaining": data["new_remaining"]},
        issued_by=claims.get("id"),
        status="pending",
    )
    db.session.add(rc)
...
def cmd_upgrade(device_id: int):
    ...
    rc = RemoteCommand(
        command_id=cmd_id,
        device_id=d.id,
        command_type=f"upgrade_{up_type}",
...
def export_devices():
    ...
    return csv_response(
        ["device_no", "model", "status", "last_seen", "address"],
        rows,
