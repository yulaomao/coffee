...
from ..models import Device, DeviceMaterial, DeviceStatusLog, MaterialCatalog
...
class DeviceRepository(BaseRepository[Device]):
    ...
    def get_devices_with_low_materials(
    ...
    def get_device_status_history(self, device_id: int, days: int = 7) -> List[DeviceStatusLog]:
        ...
        return (
            ...
            .filter(
                and_(DeviceStatusLog.device_id == device_id, DeviceStatusLog.created_at >= since)
            )
        ...
        if device:
            ...
            if old_status != status:
                status_log = DeviceStatusLog(
                    device_id=device_id, status=status, previous_status=old_status
                )
