...
from ..models import MaterialCatalog, DeviceMaterial
...
class MaterialRepository(BaseRepository[MaterialCatalog]):
    ...
    def get_material_usage_stats(self, material_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_capacity': float(usage_data.total_capacity) if usage_data.total_capacity else 0.0,
    ...
    def get_low_stock_materials(self, threshold_multiplier: float = 1.0) -> List[Dict[str, Any]]:
        ...
        return (
            self.session.query(
                ...
                func.sum(DeviceMaterial.remain).label('total_remain'),
                func.sum(DeviceMaterial.capacity).label('total_capacity'),
                func.avg(DeviceMaterial.remain / DeviceMaterial.threshold).label('avg_threshold_ratio')
            )
...
class DeviceMaterialRepository(BaseRepository[DeviceMaterial]):
    ...
    def update_material_stock(self, device_id: int, material_id: int, remain: float) -> Optional[DeviceMaterial]:
    ...
    def fill_material_stock(self, device_id: int, material_id: int, fill_amount: float = None) -> Optional[DeviceMaterial]:
    ...
    def get_materials_below_threshold(self, device_id: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        return query.order_by(
            (DeviceMaterial.remain / DeviceMaterial.threshold).asc()
        ).all()