...
from ..models import Device, Order, Product
...
class OrderRepository(BaseRepository[Order]):
    ...
    def find_by_device(self, device_id: int, limit: Optional[int] = None) -> List[Order]:
        ...
        query = (
            ...
            .filter(Order.device_id == device_id)
    ...
    def find_by_date_range(
        ...
        query = self.session.query(Order).filter(between(Order.created_at, start_date, end_date))
    ...
    def find_paid_orders(
        self, device_id: Optional[int] = None, merchant_id: Optional[int] = None
    ) -> List[Order]:
    ...
    def get_daily_sales(
        self, start_date: datetime, end_date: datetime, merchant_id: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        ...
        ).filter(and_(Order.pay_status == "paid", between(Order.created_at, start_date, end_date)))
        ...
        return (
            query.group_by(func.date(Order.created_at)).order_by(func.date(Order.created_at)).all()
        )
    ...
    def get_product_sales_stats(
        ...
        query = (
            ...
            .filter(
                and_(Order.pay_status == "paid", between(Order.created_at, start_date, end_date))
            )
        )
        ...
        return (
            query.group_by(Order.product_id, Product.name)
    ...
    def get_order_statistics(
        ...
        total_revenue = paid_query.with_entities(func.sum(Order.total_amount)).scalar() or 0
        ...
        today_revenue = (
            today_query.filter(Order.pay_status == "paid")
