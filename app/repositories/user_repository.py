...
from ..models import Merchant, Role, User
...
class UserRepository(BaseRepository[User]):
    ...
    def create_user(
        ...
        **kwargs,
        ...
        user = self.create(
            username=username, email=email, role=role, merchant_id=merchant_id, **kwargs
        )
    ...
    def get_user_statistics(self) -> Dict[str, Any]:
        ...
        stats = (
            self.session.query(User.role, User.is_active, func.count(User.id).label("count"))
            .group_by(User.role, User.is_active)
            .all()
        )

        result = {"total": 0, "active": 0, "inactive": 0, "by_role": {}}
