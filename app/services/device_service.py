...
from ..repositories import DeviceRepository, MaterialRepository, DeviceMaterialRepository
...
class DeviceService(BaseService):
    ...
    def setup_device_materials(
        self, 
        device_id: int, 
        material_configs: List[Dict[str, Any]]
    ) -> List[DeviceMaterial]:
    ...
    def refill_material(
        ...
        if not device_material:
            raise ValueError(f"Device material not found: device={device_id}, material={material_id}")
    ...
    def consume_material(
        self, 
        device_id: int, 
        material_consumptions: Dict[int, float]
    ) -> List[DeviceMaterial]:
        ...
        for material_id, consumption in material_consumptions.items():
            ...
            if device_material.remain < consumption:
                raise ValueError(f"Insufficient material {material_id}: need {consumption}, have {device_material.remain}")
