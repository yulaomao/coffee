...
from app.models import (
    User, Role, Merchant, Device, DeviceStatusLog, Order, Product,
    MaterialCatalog, DeviceMaterial, Fault, WorkOrder, DeviceBin
)
...
class DataGenerator:
    ...
    def _generate_devices(self) -> int:
        ...
        merchants = Merchant.query.filter(Merchant.name.like(f"{prefix}%")).all()
        locations = ["大厅", "二楼", "员工区", "VIP区", "休息区", "会议室", "前台", "咖啡厅", "接待室", "办公区"]
    ...
    def _generate_device_materials(self) -> int:
        ...
        materials = MaterialCatalog.query.filter_by(is_active=True).limit(
            self.config['material_variety']
        ).all()
        ...
        for device in devices:
            ...
            for material in device_materials:
                if not DeviceMaterial.query.filter_by(device_id=device.id, material_id=material.id).first():
    ...
    def _generate_orders(self) -> int:
        ...
        for device in devices:
            ...
            for _ in range(orders_count):
                ...
                pay_status = 'paid' if random.random() < 0.9 else random.choice(['pending', 'cancelled'])
...
def main():
    ...
    parser.add_argument('--scenario', choices=['production', 'development', 'demo'], 
                       default='demo', help='Scenario type')
    parser.add_argument('--force', action='store_true', help='Force regeneration (clear existing data)')
